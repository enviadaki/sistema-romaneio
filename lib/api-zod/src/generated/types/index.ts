/**
 * Generated by orval v8.5.3 🍺
 * Do not edit manually.
 * Api
 * API specification for Sistema de Romaneios
 * OpenAPI spec version: 0.1.0
 */

export * from "./arcoConfig";
export * from "./arcoLookupParams";
export * from "./arcoLookupResult";
export * from "./arcoPingResult";
export * from "./bulkImportResult";
export * from "./bulkScansInput";
export * from "./bulkScansResult";
export * from "./cityContact";
export * from "./cityCount";
export * from "./clearPackagesParams";
export * from "./clearPackagesResult";
export * from "./conferente";
export * from "./deliveryManifest";
export * from "./deliveryManifestItem";
export * from "./errorResponse";
export * from "./filialRoute";
export * from "./getRomaneioParams";
export * from "./getStatsParams";
export * from "./healthStatus";
export * from "./listFilialRoutesParams";
export * from "./listPackagesParams";
export * from "./listReturnProtocolsParams";
export * from "./listReturnProtocolsStatus";
export * from "./listScansParams";
export * from "./motorista";
export * from "./operatorCount";
export * from "./package";
export * from "./packageBulkInput";
export * from "./packageInput";
export * from "./returnProtocol";
export * from "./returnProtocolCancelInput";
export * from "./returnProtocolInput";
export * from "./returnProtocolItem";
export * from "./returnProtocolItemInput";
export * from "./returnProtocolItemInputTipo";
export * from "./returnProtocolItemTipo";
export * from "./returnProtocolStatus";
export * from "./romaneio";
export * from "./romaneioItem";
export * from "./scan";
export * from "./scanInput";
export * from "./stats";
export * from "./unscannedPackage";

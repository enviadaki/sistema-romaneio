...
export function DesignSystemBrowser() {
  ...
  const [theme, setTheme] = useState<'light' | 'dark'>(() => {
    ...
    return window.localStorage.getItem('envia-daki-theme') === 'dark'
      ? 'dark'
      : 'light';
  });
  ...
  useEffect(() => {
    ...
    window.localStorage.setItem('envia-daki-theme', theme);
  }, [theme]);
  ...
  return (
    <div className="min-h-screen bg-background text-foreground md:grid md:grid-cols-[260px_minmax(0,1fr)]">
      <aside className="border-b bg-sidebar text-sidebar-foreground md:sticky md:top-0 md:flex md:h-screen md:flex-col md:border-b-0 md:border-r md:border-sidebar-border">
        <div className="border-b px-5 py-5">
          ...
          <div className="mt-3 flex items-center justify-between gap-3">
            <p className="text-xs text-sidebar-foreground/65">Design system operacional</p>
            <Button
              type="button"
              variant="ghost"
              size="icon"
